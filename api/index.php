...
$app->get('/generatorRevisionNumbers/:date/:genID/:id/:str','getADateGenRevisionNumberSpecific');
...
function getADateGenRevisionNumberSpecific($date, $genID, $id, $str){
    $response = array();
    $db = new DbHandler();
    // fetching all users with a particular name
    $result = $db->getADateGenRevisionNumberSpecificData($date, $genID, $id, $str);
    if(gettype($result)=="string") {
        $response["error"] = true;
        $response["message"]=$result;
    }
        ...
        $response["error"] = false;
        $response["revisionNumber"] = $result;
    }
    echoResponse(200, $response);
}
